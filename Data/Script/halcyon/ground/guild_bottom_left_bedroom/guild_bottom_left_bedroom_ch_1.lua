...
function guild_bottom_left_bedroom_ch_1.Breloom_Action(chara, activator)
	...
	if not SV.Chapter1.MetBreloomGirafarig then 
		...
		UI:SetSpeaker(STRINGS:Format("\\uE040"), true, girafarig.CurrentForm.Species, girafarig.CurrentForm.Form, girafarig.CurrentForm.Skin, girafarig.CurrentForm.Gender)
		UI:SetSpeakerEmotion("Normal")
		local coro1 = TASK:BranchCoroutine(function() UI:WaitShowDialogue("C'mon,[pause=10] don't freak out the rookies.[pause=0] They haven't found out yet that you're not funny.") end)
		...
		UI:SetSpeakerEmotion("Happy")
		coro1 = TASK:BranchCoroutine(function() UI:WaitShowDialogue("Glad to have you both here in the guild![pause=0] Name's " .. breloom:GetDisplayName() .. ",[pause=10] by the way.") end)
		...
		UI:SetSpeaker(girafarig)
		UI:SetSpeakerEmotion("Happy")
		coro1 = TASK:BranchCoroutine(function() UI:WaitShowDialogue("Yeah![pause=0] We're the most senior appetizers here,[pause=10] so come to us if you need advice or help!")
												UI:WaitShowDialogue(tail:GetDisplayName() .. " and I would be glad to tutor you anytime!") end)
		coro2 = TASK:BranchCoroutine(function() GAME:WaitFrames(8)
											    GROUND:CharTurnToCharAnimated(partner, girafarig, 4)
											    GROUND:CharTurnToCharAnimated(hero, girafarig, 4) end)
		TASK:JoinCoroutines({coro1, coro2})
